...
<img src={web_render} width={500} />

从中可以看到react在web渲染完全和新架构一致并且因为没有线程切换和跨语言的资源消耗，react在一帧中就能完成上述渲染。所以rn长期以来一致在向这一方面努力，今天最起码在实现侧对齐了。

### 但是这个问题的根因是什么导致的?

在桥接渲染中，shadowtree并没有维护在c++层面，而是通过异步消息存在于java jvm等host平台中。所以如果要进行在只有第二次渲染后才能将第三次渲染同第二次渲染比较。

但是新架构该用同步架构，js直接调用api，能做到无需第三次渲染就能将最终tree给渲染出来。
